...
@Component({
  selector: 'app-Sellercheck-email',
  imports: [ReactiveFormsModule,CommonModule],
  templateUrl: './check-email.html',
  styleUrl: './check-email.css'
})
export class SellerCheckEmail implements OnInit {
  ...
  onSubmit () {
    ...
    this.authService.checkEmail({email}).subscribe({
      next: (res) => {
        ...
        if (res.isRegistered) {
          this.router.navigate(['/SellerAuth/login'], {queryParams: { email }});
        }else {
          this.router.navigate(['/SellerAuth/verification']);
        }
      },
      error: (err) => {
